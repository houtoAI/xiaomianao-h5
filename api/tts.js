...
async function getSamiToken() {
  ...
  const bodyObj = { appkey: SAMI_APPKEY, token_version: 'volc-auth-v1', expiration: 36000 };
  const bodyStr = JSON.stringify(bodyObj);
  const bodyHash256 = sha256(bodyStr);
  const headers = { 'Host': host, 'Content-Type': contentType, 'X-Date': formatDate, 'X-Content-Sha256': bodyHash256 };
  ...
  const authorization = 'HMAC-SHA256' + ' Credential=' + credential + ', SignedHeaders=' + signedHeadersStr + ', Signature=' + signature;
  const options = { hostname: host, port: 443, path: path + '?' + query, method: method, headers: { 'Host': host, 'Content-Type': contentType, 'X-Date': formatDate, 'X-Content-Sha256': bodyHash256, 'Authorization': authorization, 'Content-Length': Buffer.byteLength(bodyStr) } };
...
async function getTtsAudio(text, speaker) {
  ...
  const body = JSON.stringify({ text: text, voice_type: voiceType, audio_config: { format: 'mp3', sample_rate: 24000, speech_rate: 1.0 } });
  const options = { hostname: 'sami.bytedance.com', port: 443, path: '/api/text_to_speech?appkey=' + encodeURIComponent(SAMI_APPKEY) + '&token=' + encodeURIComponent(token), method: 'POST', headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(body) } };
